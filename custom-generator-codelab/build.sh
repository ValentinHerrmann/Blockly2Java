...
if [ "$CF_PAGES_BRANCH" != "$PROD_BRANCH" ]; then
  ...
  if [ -z "$PR_COUNT" ] || [ "$PR_COUNT" -eq 0 ]; then
    ...
    mkdir -p dist
    echo "<html><head><title>Build Skipped</title><style>body { font-family: sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #f9f9f9; color: #555; } .card { background: white; padding: 2rem; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); text-align: center; }</style></head><body><div class='card'><h2>Deployment Skipped</h2><p>This branch does not have an active, ready-for-review Pull Request.</p></div></body></html>" > dist/index.html
