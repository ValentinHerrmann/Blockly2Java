...
import '../core/events/events_block_change.ts';
...
//import type {BubbleOpen} from '../core/events/events_bubble_open.js';
import * as eventUtils from '../core/events/utils.ts';
import {Field, UnattachedFieldError} from '../core/field.ts';
import {Msg} from '../core/msg.ts';
import {Names} from '../core/names.ts';
import {IParameterModel} from '../core/interfaces/i_parameter_model.ts';
import {IProcedureMap} from '../core/interfaces/i_procedure_map.ts';
import {IProcedureModel} from '../core/interfaces/i_procedure_model.ts';
import {
  IProcedureBlock,
  isProcedureBlock,
} from '../core/interfaces/i_procedure_block.ts';
import {
  isLegacyProcedureCallBlock,
  isLegacyProcedureDefBlock,
  ProcedureBlock,
  ProcedureTuple,
} from '../core/interfaces/i_legacy_procedure_blocks.ts';
import {ObservableProcedureMap} from '../core/observable_procedure_map.ts';
import * as utilsXml from '../core/utils/xml.ts';
import * as Variables from '../core/variables.ts';
//import type {Workspace} from '../core/workspace.js';
//import type {WorkspaceSvg} from '../core/workspace_svg.js';
// import {MutatorIcon} from '../core/icons.js';
 ...
 */
export const CATEGORY_NAME = 'CONSTUCTOR';

/**
 * The default argument for a procedures_mutatorarg block.
 */
export const DEFAULT_ARG = 'a';
...
export function allProcedures(root) {
  ...
  root.getBlocksByType('constructors_defnoreturn', false).forEach((b) => {
  ...
  root.getBlocksByType('constructors_defreturn', false).forEach((b) => {
    if (!isProcedureBlock(b) && isLegacyProcedureDefBlock(b)) {
 ...
 */
export function flyoutCategory(workspace) { //: Element[]
  const xmlList = [];
  if (Blocks['constructors_defnoreturn']) {
    // <block type="constructors_defnoreturn" gap="16">
    //     <field name="NAME">do something</field>
    // </block>
    const block = utilsXml.createElement('block');
    block.setAttribute('type', 'constructors_defnoreturn');
    block.setAttribute('gap', '16');
    const nameField = utilsXml.createElement('field');
    nameField.setAttribute('name', 'NAME');
    nameField.appendChild(
      utilsXml.createTextNode(Msg['CONSTUCTORS_DEFNORETURN_PROCEDURE']),
    );
...
export function mutatorOpenListener(e) { //: Abstract
  ...
  if (type !== 'constructors_defnoreturn') {
    return;
  }
  const workspace = (
 ...
 */
export function getCallers(name, workspace) { //: Block[]
 ...
 *     procedure name.
 */
function blockIsModernCallerFor(block, procName) { //: boolean
  return (
...
export function mutateCallers(defBlock) {
  ...
  const xmlElement = defBlock.mutationToDom(true); //efBlock.mutationToDom!(true);
